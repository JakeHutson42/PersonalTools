...
(() => {
    ...
    let battleId = root.dataset.caseBattleId, detail = null, timings = { readyPauseMs:900, readyCountdownMs:3000 }, caseImages = new Map(), ownedCases = new Map(), currentReady = false, countdownShown = false, startScheduled = false, createdAt = null, refreshTimer = 0;
    ...
    const missingCaseCount = () => {
        const required = (detail?.caseKeys || []).reduce((counts, key) => { key = String(key).toLowerCase(); counts.set(key, (counts.get(key) || 0) + 1); return counts; }, new Map());
        return [...required].reduce((total, [key, quantity]) => total + Math.max(0, quantity - (ownedCases.get(key) || 0)), 0);
    };
    ...
    function render() {
        ...
        if (!detail.isParticipant) {
            const missing = missingCaseCount();
            ready.innerHTML = '<span class="case-battle-lobby-ready-icon"><i class="fa-solid fa-box-open" aria-hidden="true"></i></span><div><strong>Your seat is reserved by invitation</strong><small>' + (missing ? 'Buy ' + missing + ' missing case' + (missing === 1 ? '' : 's') + ' to join this battle.' : 'Your cases are ready. Join to claim your seat.') + '</small></div>';
            actions.innerHTML = '<button class="btn case-battle-lobby-action is-ready-action" type="button" data-lobby-action="buy-join"><i class="fa-solid ' + (missing ? 'fa-cart-shopping' : 'fa-door-open') + '" aria-hidden="true"></i><span><small>' + (missing ? missing + ' case' + (missing === 1 ? '' : 's') + ' required' : 'Loadout complete') + '</small><strong>' + (missing ? 'Buy cases &amp; join' : 'Join battle') + '</strong></span></button><button class="btn case-battle-lobby-action is-exit-action" type="button" data-lobby-action="decline"><i class="fa-solid fa-xmark" aria-hidden="true"></i><span><small>Invitation</small><strong>Decline</strong></span></button>';
        } else {
            actions.innerHTML = '<button class="btn case-battle-lobby-action is-ready-action" type="button" data-lobby-action="ready"><i class="fa-solid ' + (currentReady ? 'fa-pause' : 'fa-bolt') + '" aria-hidden="true"></i><span><small>' + (currentReady ? 'Status locked' : 'Confirm loadout') + '</small><strong>' + (currentReady ? 'Cancel ready' : 'Ready up') + '</strong></span></button>' + (detail.isCreator ? '<button class="btn case-battle-lobby-action is-exit-action" type="button" data-lobby-action="cancel"><i class="fa-solid fa-xmark" aria-hidden="true"></i><span><small>Owner control</small><strong>Cancel battle</strong></span></button>' : '<button class="btn case-battle-lobby-action is-exit-action" type="button" data-lobby-action="leave"><i class="fa-solid fa-arrow-right-from-bracket" aria-hidden="true"></i><span><small>Exit room</small><strong>Leave lobby</strong></span></button>');
        }
    ...
    actions.addEventListener('click', event => {
        ...
        if (action === 'buy-join' || action === 'decline') {
            const missingBeforeJoin = missingCaseCount();
            request('/api/case-battles/' + encodeURIComponent(battleId) + '/invite/' + (action === 'buy-join' ? 'buy-and-accept' : 'decline'), 'POST')
                .then(() => { if (action === 'decline') { personalToolsToast?.success('Invitation declined.'); window.caseBattleTransition?.navigate('/CaseOpening'); return; } personalToolsToast?.success(missingBeforeJoin ? 'Cases purchased and seat confirmed.' : 'Seat confirmed.'); return load(); })
                .catch(message => { button.disabled = false; personalToolsToast?.error(message || 'The invitation could not be updated.'); });
            return;
        }
    ...
    Promise.all([load(), fetch('/api/case-battles/timings', { credentials:'same-origin', cache:'no-store' }).then(response => response.ok ? response.json() : null), fetch('/api/case-opening/cases', { credentials:'same-origin', cache:'no-store' }).then(response => response.ok ? response.json() : [])]).then(([, settings, cases]) => { if (settings) timings = { ...timings, ...settings }; (cases || []).forEach(item => { const key = String(item.caseKey || '').toLowerCase(); caseImages.set(key, item.imageUrl || ''); ownedCases.set(key, Number(item.ownedQuantity || 0)); }); render(); window.caseBattleTransition?.complete(); connectRealtime(); }).catch(() => window.caseBattleTransition?.complete());
})();
