...
export function createChessAnalysis(element, preview = {}) {
    ...
    function liveMessage(label, detail, quality = '') {
        if (!element) return;
        element.hidden = false; element.className = `chess-analysis ${quality}`; element.replaceChildren();
    ...
    function select(ply) { if (!reviewGame?.moves?.[ply]) return; selectedPly = ply; renderGraph(); renderInsight(); }
    ...
    function quickMoveFeedback(game) {
        ...
        if (ply < 0 || !isMine(game.moves[ply])) { liveMessage('Move feedback', 'The computer is thinking. Your move will be checked after it replies.'); return; }
