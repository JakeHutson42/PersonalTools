...
public interface ISocialProfileData
{
    ...
    Task<List<SocialProfileObj>> GetPendingFriendRequests(Guid userId, CancellationToken cancellationToken = default);
...
public sealed class SocialProfileData(IMariaDbDataAccess database) : ISocialProfileData
{
    ...
    public Task<List<SocialProfileObj>> GetPendingFriendRequests(Guid userId, CancellationToken cancellationToken = default) =>
        database.GetBulkDataSP("sp_social_friend_requests_get", Read, Params(("p_user_id", userId)), cancellationToken);
    ...
    public Task AcceptFriendRequest(Guid userId, Guid requesterUserId, CancellationToken cancellationToken = default) =>
        database.ExecuteSP("sp_social_friend_request_accept", Params(("p_user_id", userId), ("p_requester_user_id", requesterUserId)), cancellationToken);
    public Task DenyFriendRequest(Guid userId, Guid requesterUserId, CancellationToken cancellationToken = default) =>
        database.ExecuteSP("sp_social_friend_request_deny", Params(("p_user_id", userId), ("p_requester_user_id", requesterUserId)), cancellationToken);
    ...
    private static SocialProfileObj Read(MySqlDataReader reader) => new()
    {
        ...
        LastSeenUtc = reader.IsDBNull(reader.GetOrdinal("LastSeenUtc")) ? null : DateTime.SpecifyKind(reader.GetDateTime("LastSeenUtc"), DateTimeKind.Utc),
        IsFriend = reader.GetBoolean("IsFriend"),
        HasOutgoingFriendRequest = HasColumn(reader, "HasOutgoingFriendRequest") && reader.GetBoolean("HasOutgoingFriendRequest"),
        HasIncomingFriendRequest = HasColumn(reader, "HasIncomingFriendRequest") && reader.GetBoolean("HasIncomingFriendRequest")
    };
    private static bool HasColumn(MySqlDataReader reader, string name) { for (int index = 0; index < reader.FieldCount; index++) if (reader.GetName(index).Equals(name, StringComparison.OrdinalIgnoreCase)) return true; return false; }
