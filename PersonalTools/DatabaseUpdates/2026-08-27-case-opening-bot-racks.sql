-- Compact bot racks, individually-upgradeable bots and persistent server power state.
...
-- Preserve existing server progression by spreading its twenty legacy levels over four bots.
-- The nested SELECT avoids MariaDB's target-table update restriction; signed arithmetic avoids
-- unsigned underflow on racks whose later slots receive zero migrated levels.
