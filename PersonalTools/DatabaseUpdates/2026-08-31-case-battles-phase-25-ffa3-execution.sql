-- Phase 2 enables server execution and winner-takes-all settlement for duel and ffa-3.
USE PersonalTools;
DELIMITER //
...
BEGIN
    ...
    IF NOT EXISTS(SELECT 1 FROM CaseOpeningBattles WHERE BattleId=p_battle_id AND Status='waiting' AND Mode='duel')
       OR (SELECT COUNT(*) FROM CaseOpeningBattleParticipants WHERE BattleId=p_battle_id)<>1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Battle Bot can only join an empty 1v1 opponent seat.';
    END IF;
...
settle: BEGIN
    ...
    IF BINARY v_winner=BINARY '00000000-0000-0000-0000-000000000001' THEN
        ...
        SET BattlesWon=BattlesWon+1,SkinsDiscarded=SkinsDiscarded+v_rolls,
            ValueDiscarded=ValueDiscarded+(SELECT COALESCE(SUM(LockedValue),0) FROM CaseOpeningBattleRolls WHERE BattleId=p_battle_id),UpdatedUtc=UTC_TIMESTAMP(6)
        WHERE StatsId=1;
    ...
    SELECT GREATEST(capacity.BaseCapacity
        +COALESCE((SELECT SUM(AddedSlots) FROM CaseOpeningStorageContainers WHERE UserId=v_winner),0)
        +COALESCE(upgrades.BonusInventorySlots,0)
        -(SELECT COUNT(*) FROM CaseOpeningHistory WHERE UserId=v_winner)
        -(SELECT COALESCE(SUM(Quantity),0) FROM CaseOpeningOwnedCases WHERE UserId=v_winner),0)
    INTO v_capacity
    FROM CaseOpeningInventoryCapacity capacity
    LEFT JOIN CaseOpeningUserInventoryUpgrades upgrades ON upgrades.UserId=capacity.UserId
    WHERE capacity.UserId=v_winner FOR UPDATE;
    ...
    INNER JOIN CaseOpeningBattleSettlements settlement ON settlement.BattleRollId=roll.BattleRollId
    ...
    WHERE participant.BattleId=p_battle_id;
    UPDATE CaseOpeningBattleOverflow overflowRow
    INNER JOIN CaseOpeningBattleOverflowReservations reservation ON reservation.UserId=overflowRow.UserId
    SET overflowRow.OccupiedSlots=overflowRow.OccupiedSlots+IF(reservation.UserId=v_winner,v_overflow,0),
        overflowRow.UpdatedUtc=UTC_TIMESTAMP(6),reservation.OccupiedSlots=IF(reservation.UserId=v_winner,v_overflow,0),reservation.ReleasedUtc=UTC_TIMESTAMP(6)
