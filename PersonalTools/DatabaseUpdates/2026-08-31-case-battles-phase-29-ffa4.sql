-- Phase 29 adds four-player free-for-all battles and a third system bot for testing.
USE PersonalTools;

ALTER TABLE CaseOpeningBattleBotSettings
    ADD COLUMN IF NOT EXISTS FreeForAll4Enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER FreeForAll3Enabled;

INSERT IGNORE INTO Users(UserId,Email,DisplayName,PasswordHash,IsActive,UserRole,CreatedUtc)
VALUES('00000000-0000-0000-0000-000000000003','battle-bot-3@system.invalid','Battle Bot Charlie','SYSTEM-ACCOUNT',0,1,UTC_TIMESTAMP(6));
...
BEGIN
    SELECT settings.CaseBattlesEnabled,settings.FreeForAll3Enabled,settings.FreeForAll4Enabled,settings.Enabled,
...
DROP PROCEDURE IF EXISTS sp_case_battles_ffa4_enabled_set//
CREATE PROCEDURE sp_case_battles_ffa4_enabled_set(IN p_enabled TINYINT)
BEGIN
    UPDATE CaseOpeningBattleBotSettings
    SET FreeForAll4Enabled=IF(p_enabled<>0,1,0),UpdatedUtc=UTC_TIMESTAMP(6)
    WHERE SettingsId=1;
END//
...
BEGIN
    ...
    SET v_bot_count=CASE v_mode WHEN 'duel' THEN 1 WHEN 'ffa-3' THEN 2 WHEN 'ffa-4' THEN 3 ELSE 0 END;
    ...
    SELECT '00000000-0000-0000-0000-000000000001',CaseKey,COUNT(*),UTC_TIMESTAMP(6) FROM CaseOpeningBattleCases WHERE BattleId=p_battle_id GROUP BY CaseKey
    ...
    IF v_bot_count>=2 THEN
        INSERT INTO CaseOpeningOwnedCases(UserId,CaseKey,Quantity,UpdatedUtc)
        SELECT '00000000-0000-0000-0000-000000000002',CaseKey,COUNT(*),UTC_TIMESTAMP(6) FROM CaseOpeningBattleCases WHERE BattleId=p_battle_id GROUP BY CaseKey
        ON DUPLICATE KEY UPDATE Quantity=CaseOpeningOwnedCases.Quantity+VALUES(Quantity),UpdatedUtc=VALUES(UpdatedUtc);
    ...
    IF v_bot_count>=3 THEN
        INSERT INTO CaseOpeningOwnedCases(UserId,CaseKey,Quantity,UpdatedUtc)
        SELECT '00000000-0000-0000-0000-000000000003',CaseKey,COUNT(*),UTC_TIMESTAMP(6) FROM CaseOpeningBattleCases WHERE BattleId=p_battle_id GROUP BY CaseKey
        ON DUPLICATE KEY UPDATE Quantity=CaseOpeningOwnedCases.Quantity+VALUES(Quantity),UpdatedUtc=VALUES(UpdatedUtc);
        CALL sp_case_battles_join(p_battle_id,'00000000-0000-0000-0000-000000000003');
        UPDATE CaseOpeningBattleParticipants SET IsReady=1 WHERE BattleId=p_battle_id AND UserId='00000000-0000-0000-0000-000000000003';
    END IF;
...
settle: BEGIN
    ...
    IF v_winner IN ('00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000002','00000000-0000-0000-0000-000000000003') THEN
    ...
    END IF;
    SET v_allocation_reason=CONCAT(v_mode,' winner');
