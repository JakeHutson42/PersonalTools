...
public sealed class CaseBattlesAdminController(ICaseBattleFuncs battles, ILogger<CaseBattlesAdminController> logger) : ControllerBase
{
    ...
    [HttpPut("ffa-4-status")] public async Task<ActionResult<CaseBattleBotStatusObj>> SetFreeForAll4Status([FromBody] bool enabled, CancellationToken cancellationToken) { await battles.SetFreeForAll4Enabled(enabled, cancellationToken); return Ok(await battles.GetBotStatus(cancellationToken)); }
